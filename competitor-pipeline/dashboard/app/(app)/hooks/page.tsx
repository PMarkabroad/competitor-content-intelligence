  ...
  return (
    <div className="p-4">
      <div className="mb-4 flex items-baseline justify-between">
        <div>
          <h1 className="text-sm font-semibold text-[var(--color-text)]">Hook library</h1>
          <p className="text-xs text-[var(--color-text-dim)]">
            {rows.length} hook(s){hiddenCount > 0 && !showNo ? ` · ${hiddenCount} brand_fit=no hidden` : ""}
          </p>
        </div>
        <form method="get" className="flex items-center gap-2">
          <select name="hook_pattern" defaultValue={params.hook_pattern ?? ""} className="rounded border border-[var(--color-border)] bg-[var(--color-bg-raised)] px-2 py-1 text-xs text-[var(--color-text)]">
            <option value="">All patterns</option>
            {HOOK_PATTERNS.map((p) => <option key={p} value={p}>{p}</option>)}
          </select>
          <select name="topic_slug" defaultValue={params.topic_slug ?? ""} className="rounded border border-[var(--color-border)] bg-[var(--color-bg-raised)] px-2 py-1 text-xs text-[var(--color-text)]">
            <option value="">All topics</option>
            {TOPIC_SLUGS.map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
          <select name="brand_fit" defaultValue={params.brand_fit ?? ""} className="rounded border border-[var(--color-border)] bg-[var(--color-bg-raised)] px-2 py-1 text-xs text-[var(--color-text)]">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2 xl:grid-cols-3">
        {rows.map((row) => {
          ...
          return (
            <div key={row.hook_id} className="rounded border border-[var(--color-border)] bg-[var(--color-bg-raised)] p-3">
              <div className="mb-2 flex items-start justify-between gap-2">
                <p className="text-sm font-medium text-[var(--color-text)]">&ldquo;{row.opening_line}&rdquo;</p>
                <span className="shrink-0 font-mono text-sm font-semibold text-[var(--color-brand)]">{formatScore(row.outlier_score)}</span>
              </div>
              ...
              {row.au_transplant && (
                <p className="mb-1 text-xs text-[var(--color-text-dim)]">
                  <span className="text-[var(--color-text-faint)]">au_transplant:</span> {row.au_transplant}
                  {row.transplant_note && <span className="text-[var(--color-text-faint)]"> — {row.transplant_note}</span>}
                </p>
              )}
              {row.brand_fit_note && (
                <p className="mb-2 text-xs text-[var(--color-text-dim)]">
                  <span className="text-[var(--color-text-faint)]">brand_fit note:</span> {row.brand_fit_note}
                </p>
              )}
              <div className="flex items-center gap-3">
                {row.competitor_posts?.post_url && (
                  <a href={row.competitor_posts.post_url} target="_blank" rel="noreferrer" className="text-xs text-[var(--color-brand)] hover:underline">
                    Original post →
                  </a>
                )}
                {transcript && (
                  <details className="text-xs">
                    <summary className="cursor-pointer text-[var(--color-text-dim)] hover:text-[var(--color-text)]">Full transcript</summary>
                    <p className="mt-2 whitespace-pre-wrap text-[var(--color-text-dim)]">{transcript}</p>
                  </details>
                )}
              </div>
            </div>
